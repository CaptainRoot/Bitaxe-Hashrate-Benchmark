...
if initial_voltage > max_allowed_voltage:
    raise ValueError(
        RED
        + f"Error: Initial voltage exceeds the maximum allowed value of {max_allowed_voltage}mV. Please check the input and try again."
        + RESET
    )
...
if initial_frequency > max_allowed_frequency:
    raise ValueError(
        RED
        + f"Error: Initial frequency exceeds the maximum allowed value of {max_allowed_frequency}Mhz. Please check the input and try again."
        + RESET
    )
...
if initial_voltage < min_allowed_voltage:
    raise ValueError(
        RED
        + f"Error: Initial voltage is below the minimum allowed value of {min_allowed_voltage}mV."
        + RESET
    )
...
if initial_frequency < min_allowed_frequency:
    raise ValueError(
        RED
        + f"Error: Initial frequency is below the minimum allowed value of {min_allowed_frequency}MHz."
        + RESET
    )
...
if benchmark_time / sample_interval < 7:
    raise ValueError(
        RED
        + f"Error: Benchmark time is too short. Please increase the benchmark time or decrease the sample interval. At least 7 samples are required."
        + RESET
    )
...
def fetch_default_settings():
    ...
    try:
        ...
        if "smallCoreCount" not in system_info:
            print(
                RED
                + "Error: smallCoreCount field missing from /api/system/info response."
                + RESET
            )
            print(
                RED
                + "Cannot proceed without core count information for hashrate calculations."
                + RESET
            )
            sys.exit(1)
        ...
        if has_voltage and has_frequency and has_asic_count:
            ...
            print(
                GREEN + f"Current settings determined from /api/system/info:\n"
            ...
            print(
                YELLOW
                + f"Got small_core_count ({small_core_count}) from /api/system/info, getting remaining info from /api/system/asic..."
                + RESET
            )
    ...
    try:
        ...
        print(
            GREEN + f"Current settings determined from /api/system/asic:\n"
            f"  Core Voltage: {default_voltage}mV\n"
            f"  Frequency: {default_frequency}MHz\n"
    ...
    print(
        RED
        + "Failed to fetch rest of the device information from /api/system/asic."
        + RESET
    )
    print(
        RED
        + "Cannot proceed safely without device configuration. Please check your connection and try again."
        + RESET
    )
    sys.exit(1)
...
def get_system_info():
    ...
    for attempt in range(retries):
        try:
            ...
            print(
                YELLOW
                + f"Timeout while fetching system info. Attempt {attempt + 1} of {retries}."
                + RESET
            )
        except requests.exceptions.ConnectionError:
            print(
                RED
                + f"Connection error while fetching system info. Attempt {attempt + 1} of {retries}."
                + RESET
            )
        except requests.exceptions.RequestException as e:
...
def set_system_settings(core_voltage, frequency):
    ...
    try:
        ...
        print(
            YELLOW
            + f"Applying settings: Voltage = {core_voltage}mV, Frequency = {frequency}MHz"
            + RESET
        )
        time.sleep(2)
...
def restart_system():
    try:
        ...
        if not is_interrupt:
            print(
                YELLOW
                + f"Applying new settings and waiting {sleep_time}s for system stabilization..."
                + RESET
            )
                ...
                try:
                    ...
                    if info:
                        ...
                        if temp and temp >= max_temp:
                            print(
                                RED
                                + f"CRITICAL: Chip temperature {temp}°C exceeded limit {max_temp}°C during stabilization!"
                                + RESET
                            )
                            return False
                        ...
                        if vr_temp and vr_temp >= max_vr_temp:
                            print(
                                RED
                                + f"CRITICAL: VR temperature {vr_temp}°C exceeded limit {max_vr_temp}°C during stabilization!"
                                + RESET
                            )
                            return False
                        ...
                        if power and power > max_power:
                            print(
                                RED
                                + f"CRITICAL: Power {power}W exceeded limit {max_power}W during stabilization!"
                                + RESET
                            )
                            return False
...
def benchmark_iteration(core_voltage, frequency):
    ...
    print(
        GREEN
        + f"[{current_time}] Starting benchmark for Core Voltage: {core_voltage}mV, Frequency: {frequency}MHz"
        + RESET
    )
    hash_rates = []
    s1 = 0.0
    ...
    for sample in range(total_samples):
        ...
        if info is None:
            print(
                YELLOW
                + "Skipping this iteration due to failure in fetching system info."
                + RESET
            )
            return (
                None,
        ...
        if temp is None:
            ...
            return (
                ...
                None,
                None,
                "TEMPERATURE_DATA_FAILURE",
            )

        if temp < 5:
            print(
                YELLOW
                + "Temperature is below 5°C. This is unexpected. Please check the system."
                + RESET
            )
            return (
                None,
        ...
        if temp >= max_temp:
            print(
                RED
                + f"Chip temperature exceeded {max_temp}°C! Stopping current benchmark."
                + RESET
            )
        ...
        if vr_temp is not None and vr_temp >= max_vr_temp:
            print(
                RED
                + f"Voltage regulator temperature exceeded {max_vr_temp}°C! Stopping current benchmark."
                + RESET
            )
        ...
        if voltage < min_input_voltage:
            print(
                RED
                + f"Input voltage is below the minimum allowed value of {min_input_voltage}mV! Stopping current benchmark."
                + RESET
            )
            return (
                None,
        ...
        if voltage > max_input_voltage:
            print(
                RED
                + f"Input voltage is above the maximum allowed value of {max_input_voltage}mV! Stopping current benchmark."
                + RESET
            )
            return (
                None,
        ...
        if hash_rate is None or power_consumption is None:
            ...
            return (
                ...
                None,
                None,
                "HASHRATE_POWER_DATA_FAILURE",
            )

        if power_consumption > max_power:
            print(
                RED
                + f"Power consumption exceeded {max_power}W! Stopping current benchmark."
                + RESET
            )
            return (
                None,
    ...
    if hash_rates and temperatures and power_consumptions:
        ...
        hashrate_stdev = (
            statistics.stdev(trimmed_hashrates) if len(trimmed_hashrates) > 1 else 0.0
        )
        ...
        if average_hashrate > 0:
            ...
            print(
                RED
                + "Warning: Zero hashrate detected, skipping efficiency calculation"
                + RESET
            )
        ...
        print(
            GREEN
            + f"Average Hashrate: {average_hashrate:.2f} GH/s (Expected: {expected_hashrate:.2f} GH/s)"
            + RESET
        )
...
def reset_to_best_setting():
    if not results:
        ...
        best_result = sorted(results, key=lambda x: x["averageHashRate"], reverse=True)[
            0
        ]
        ...
        print(
            GREEN
            + f"Best Hashrate Settings: {best_voltage}mV / {best_frequency}MHz ({best_result['averageHashRate']:.2f} GH/s)"
            + RESET
        )
        print(
            GREEN
            + f"Most Efficient Settings: {eff_voltage}mV / {eff_frequency}MHz ({efficient_result['efficiencyJTH']:.2f} J/TH)"
            + RESET
        )

        print(
            YELLOW
            + "\nWARNING: 'Best Hashrate' settings are often at the thermal/stability limit."
            + RESET
        )
        print(
            YELLOW + "Running these settings 24/7 may reduce hardware lifespan." + RESET
        )
        print(
            YELLOW
            + "Applying 'Most Efficient' settings is generally safer for long-term use."
            + RESET
        )
...
if args.set_values:
    ...
    print(
        GREEN
        + f"Applying Core Voltage: {initial_voltage}mV, Frequency: {initial_frequency}MHz to Bitaxe."
        + RESET
    )
    ...
    if set_system_settings(initial_voltage, initial_frequency):
        print(
            GREEN
            + "Settings applied. Check your Bitaxe web interface to confirm."
            + RESET
        )
        sys.exit(0)
    else:
        print(
            RED
            + "Failed to apply settings or stabilization failed. Check your Bitaxe."
            + RESET
        )
        sys.exit(1)
...
try:
    ...
    print(RED + "\nDISCLAIMER:" + RESET)
    print(
        "This tool will stress test your Bitaxe by running it at various voltages and frequencies."
    )
    print(
        "While safeguards are in place, running hardware outside of standard parameters carries inherent risks."
    )
    print(
        "Use this tool at your own risk. The author(s) are not responsible for any damage to your hardware."
    )
    print(
        "\nNOTE: Ambient temperature significantly affects these results. The optimal settings found may not"
    )
    print(
        "work well if room temperature changes substantially. Re-run the benchmark if conditions change.\n"
    )
        ...
        if (
            ...
            if hashrate_ok:
                ...
                if current_frequency + frequency_increment <= max_allowed_frequency:
                    ...
                    print(
                        GREEN
                        + "Hashrate is good. Increasing frequency for next try."
                        + RESET
                    )
                else:
                    print(
                        GREEN
                        + "Reached max frequency with good results. Stopping further testing."
                        + RESET
                    )
                ...
                if current_voltage + voltage_increment <= max_allowed_voltage:
                    ...
                    print(
                        YELLOW
                        + f"Hashrate too low compared to expected. Adjusting to {current_frequency}MHz and increasing voltage to {current_voltage}mV"
                        + RESET
                    )
                else:
                    print(
                        YELLOW
                        + "Reached max voltage without good results. Stopping further testing."
                        + RESET
                    )
            ...
            if (
                ...
                and current_frequency == initial_frequency
            ):
                print(
                    RED
                    + "Overheated at initial frequency! Cannot increase voltage safely. Stopping."
                    + RESET
                )
                break
            ...
            if overheat_retry_allowed:
                ...
                if current_voltage + voltage_increment <= max_allowed_voltage:
                    ...
                    print(
                        GREEN
                        + "Reached thermal limit for the current voltage/frequency. Switching to next voltage increment."
                        + RESET
                    )
                else:
                    print(
                        GREEN
                        + "Reached thermal limit for the current voltage/frequency. Next voltage increment out of voltage limit. Stopping further testing."
                        + RESET
                    )
    ...
    if valid_results:
            ...
            "top_performers": [
                {
                    ...
                    **(
                        {"averageVRTemp": result["averageVRTemp"]}
                        if "averageVRTemp" in result
                        else {}
                    ),
                    **(
                        {"averageFanSpeed": result["averageFanSpeed"]}
        ...
        if top_5_results:
            ...
            for i, result in enumerate(top_5_results, 1):
                ...
                print(
                    GREEN
                    + f"  Hashrate Std Dev: {result.get('hashrateStdDev', 0.0):.2f} GH/s"
                    + RESET
                )
                print(
                    GREEN
                ...
                print(
                    GREEN + f"  Efficiency: {result['efficiencyJTH']:.2f} J/TH" + RESET
                )
                print(
                    GREEN + f"  Average Power: {result['averagePower']:.2f} W" + RESET
                )
                if "averageFanSpeed" in result:
                    print(
                        GREEN
                        + f"  Average Fan Speed: {result['averageFanSpeed']:.2f}%"
                        + RESET
                    )
                if "averageVRTemp" in result:
                    print(
                        GREEN
                        + f"  Average VR Temperature: {result['averageVRTemp']:.2f}°C"
                        + RESET
                    )
